...
class TrimFilter(FilterProcessor):
    ...
    def get_foreground_bbox(
        ...
        if np is None:
            return self._get_foreground_bbox_without_numpy(
                image=image,
                threshold=threshold,
                padding=padding,
                trim_left=trim_left,
    ...
    def _get_foreground_bbox_without_numpy(
            self,
            image: Image.Image,
            threshold: float = 10.0,
            padding: int = 0,
        ...
        background = tuple(sum(color[idx] for color in corners) / len(corners) for idx in range(4))

        def exceeds_threshold(x: int, y: int) -> bool:
            pixel = pixels[x, y]
            diff = sum((float(pixel[idx]) - background[idx]) ** 2 for idx in range(4)) ** 0.5
            return diff > threshold

        left = 0
        while left < width and not any(exceeds_threshold(left, y) for y in range(height)):
            left += 1
        ...
        right = width - 1
        while right >= 0 and not any(exceeds_threshold(right, y) for y in range(height)):
            right -= 1

        top = 0
        while top < height and not any(exceeds_threshold(x, top) for x in range(width)):
            top += 1

        bottom = height - 1
        while bottom >= 0 and not any(exceeds_threshold(x, bottom) for x in range(width)):
            bottom -= 1
...
class ShadowFilter(FilterProcessor):
    ...
    def process(self, ctx: PipelineContext):
        ...
        for img in ctx.get_buffer():
            ...
            shadow_blurred = (
                self._apply_alpha_falloff_without_numpy(shadow_blurred, falloff)
                if np is None else
                self._apply_alpha_falloff(shadow_blurred, falloff)
            )
            # 4. 合成原图
    ...
    def _apply_alpha_falloff_without_numpy(self, img: Image.Image, gamma: float) -> Image.Image:
        r, g, b, a = img.split()
        new_alpha = a.point(
            lambda value: 0
            if ((value / 255.0) ** gamma) < 0.01
            else int(((value / 255.0) ** gamma) * 255)
        )
