    ...
    try:
        if font_path:
            ...
            for fallback in [fonts_dir / "AlibabaPuHuiTi-2-45-Light.otf", "arial.ttf", "Arial.ttf", "DejaVuSans.ttf"]:
                try:
                    return ImageFont.truetype(str(fallback), BASE_FONT_SIZE)
                except OSError:
                    continue
            else:
...
def _apply_easing_scalar(t: float, method: InterpolateMethod) -> float:
    if method == InterpolateMethod.EASE_IN:
        return t * t
    if method == InterpolateMethod.EASE_OUT:
        return 1 - (1 - t) ** 2
    if method == InterpolateMethod.EASE_IN_OUT:
        return 2 * t * t if t < 0.5 else 1 - (-2 * t + 2) ** 2 / 2
    return t


def _draw_gradient_without_numpy(
        width: int,
        height: int,
        start_rgba: tuple,
        end_rgba: tuple,
    ...
    max_radial_distance = (center_x ** 2 + center_y ** 2) ** 0.5 or 1.0
    ...
    for y in range(height):
        for x in range(width):
            if direction == Direction.HORIZONTAL:
                ...
                denominator = width + height - 2
                t = (x + y) / denominator if denominator > 0 else 0.0
            elif direction == Direction.RADIAL:
                distance = ((x - center_x) ** 2 + (y - center_y) ** 2) ** 0.5
                t = min(distance / max_radial_distance, 1.0)
            else:
                t = 0.0

            t = _apply_easing_scalar(t, method)
            pixels[x, y] = tuple(
                int(start_rgba[idx] + (end_rgba[idx] - start_rgba[idx]) * t)
                for idx in range(4)
            )
...
def _draw_gradient_numpy(
    ...
    if np is None:
        return _draw_gradient_without_numpy(width, height, start_rgba, end_rgba, direction, method)
